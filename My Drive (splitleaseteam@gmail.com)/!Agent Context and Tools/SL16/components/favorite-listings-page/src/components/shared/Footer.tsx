/**
 * Footer Component
 * Reusable footer from footer-index repository
 */
...
const Footer: React.FC<FooterProps> = ({
  ...
  const handleReferralSubmit = async () => {
    if (!referralContact.trim()) return;
  ...
  const handleImportSubmit = async () => {
    if (!importUrl.trim() || !importEmail.trim()) return;

    if (!importUrl.startsWith('http://') && !importUrl.startsWith('https://')) return;
    if (!importEmail.includes('@') || !importEmail.includes('.')) return;
